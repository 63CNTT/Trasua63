...
namespace Trasua.Areas.Admin.Controllers
{
    public class NguoidungsController : Controller
    {
        ...
        public ActionResult Create([Bind(Include = "MaNguoiDung,Hoten,Email,Dienthoai,Matkhau,IDQuyen,Anhdaidien")] Nguoidung nguoidung)
        {
            if (ModelState.IsValid)
            {
				if (db.Nguoidungs.Any(c => c.Email == nguoidung.Email))
				{
					ViewBag.Fail = "Địa chỉ email đã được sử dụng. Vui lòng chọn một địa chỉ email khác.";
					return View(nguoidung);
				}
				nguoidung.Anhdaidien = Url.Content("~/Images/files/user-avatar.png");
